...
function _setCPU() {
	...
	_msgOpt "1) AMD"
	_msgOpt "2) Intel"
	_msgOpt "*) Intel - No Vulkan (IvyBridge)"
	read -p '?: ' -e optCpuType
...
function _setSwapSize() {
	_msg "Swap Size:"
	_msgOpt "1) 4GB"
	_msgOpt "2) 16GB"
	_msgOpt "*) 8GB"
	read -p "?: " -e optSwapSize
...
function _setConfirm() {
	...
	_msgOpt "CPU: $cpuType"
	_msgOpt "GPU: $gpuType"
	_msgOpt "Swap: $swapSize"
	_msgOpt "Profile: $profile"

	echo; _msgAlert "Confirm (y/N)"
	read -p "?: " -e setConfirm
	[ ! "$setConfirm" == "y" ] && echo && _msgAlert "Aborting" && exit 1
...
function _initSetup() {
	...
	_setProfileIF # utils/profiles/${IFprofile}.sh
...
function _instPacstrap() {
	...
	pacman -Syy --config "$pacmanConf" 1> /dev/null
	pacstrap -KMC "$pacmanConf" /mnt base linux-lts linux-firmware mkinitcpio grub
...
function _instFiles() {
	...
	install -Dm 644 -t /mnt/gChroot/profiles/ profiles/*
	...
	echo "#!/bin/bash
		...
		IFprofile=${IFprofile}" > /mnt/gChroot/utils/chrConf.sh
...
function _instFstab() {
	...
	mkswap -U clear -s $swapSize -F /mnt/swapfile

	printf "# /swapfile\n/swapfile	swap	swap	defaults,relatime 0 0\n" | tee -a /mnt/etc/fstab 1> /dev/null
