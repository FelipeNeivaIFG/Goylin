...
function _prepare() {
	...
	[ ! -d "/srv/http/${wRepoName}" ] && mkdir "/srv/http/$wRepoName"
	[ ! -f "/srv/http/${wRepoName}/${wRepoName}.db.tar.gz" ] && touch "/srv/http/${wRepoName}/${wRepoName}.db.tar.gz"
...
function _bootstrapChroot() {
	...
	mkarchroot -C "../../${pacmanConf}" "${pkgRoot}/root" base base-devel git 1> /dev/null
...
function _buildPKG() {
	...
	# In case of PGP key error:
	# makechrootpkg -c -r "${pkgRoot}" -- --skippgpcheck #1> /dev/null
