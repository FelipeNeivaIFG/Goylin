...
function _bootstrapChroot() {
    ...
    mkdir -p ${chroot}
    mkarchroot -C ../pacman_local.conf ${chroot}/root base-devel 1> /dev/null
    sudo cp ../pacman_local.conf ${chroot}/root/etc/pacman.conf
}
...
function _addToRepo() {
    echo; _msgInfo "Adding ${pkgName} to repository"; echo
